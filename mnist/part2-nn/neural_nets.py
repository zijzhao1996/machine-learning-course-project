...
#pragma: coderesponse template prefix="class NeuralNetwork(NeuralNetworkBase):\n\n"
    def train(self, x1, x2, y):
        ...
        grad = np.asscalar(activated_output-y)

        bias_gradients = grad*output_layer_activation_derivative(output)*np.dot(self.hidden_to_output_weights,np.vectorize(rectified_linear_unit_derivative)(hidden_layer_activation))
        hidden_to_output_weight_gradients = grad*hidden_layer_activation.T
        input_to_hidden_weight_gradients = grad*self.hidden_to_output_weights*np.vectorize(rectified_linear_unit_derivative)(hidden_layer_activation)
        input_to_hidden_weight_gradients = (np.asscalar(input_to_hidden_weight_gradients)*input_values).T
