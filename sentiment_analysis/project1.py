...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    each_loss = 1-np.multiply(np.dot(feature_matrix,theta)+theta_0, labels)
    each_loss = np.array([x if x>0 else 0 for x in each_loss])
    return np.sum(each_loss)/feature_matrix.shape[0]
#pragma: coderesponse end
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            sum_theta += theta;
            sum_theta_0 += theta_0;
    theta = np.zeros(feature_matrix.shape[1]) + sum_theta/(feature_matrix.shape[0]*T)
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Your code here
    theta = np.transpose(theta)
    each_prediction = np.dot(feature_matrix,theta)+theta_0
    each_prediction = [1 if x>0 else -1 for x in each_prediction]
    return np.array(each_prediction)
#pragma: coderesponse end
...
def bag_of_words(texts):
    ...
    stopwords = extract_words(stopwords)
...
def extract_bow_feature_vectors(reviews, dictionary):
    ...
    for i, text in enumerate(reviews):
        word_list = extract_words(text)
        for word in word_list:
            if word in dictionary and feature_matrix[i, dictionary[word]] == 0:
                feature_matrix[i, dictionary[word]] = 1
            elif word in dictionary and feature_matrix[i, dictionary[word]] >= 1:
                feature_matrix[i, dictionary[word]] += 1
